...
func TestRepositoryTmuxRunnerIsolatesServerCodexStateAndPrompt(t *testing.T) {
	...
	cfgB := runtimeIsolationTestConfig(t, "acme", "craigbot")
	...
	handleA, err := runnerA.Start(agentA, "review Mini-Me PR #156")
	if err != nil {
		t.Fatalf("Start(A) error = %v", err)
	}
	handleB, err := runnerB.Start(agentB, "review Craigbot PR #156")
	if err != nil {
	...
	if !strings.Contains(body, "review Mini-Me PR #156") {
		t.Fatalf("initial prompt body = %q, want Mini-Me task", body)
	}
	if strings.Contains(body, "Craigbot") {
	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(candidate *RuntimeHandle) {
				candidate.TmuxServer = "rao-acme-craigbot-foreign"
			},
		},
		{
			name: "instance",
		...
		},
		{
			name: "repository",
			mutate: func(candidate *RuntimeHandle) {
				candidate.Scope.RepoName = "craigbot"
			},
		},
		{
