...
// one reviewer's verdict. It folds in aggregationDigest (issue #140) so two
...
func prepareReviewVerdictPublication(
	...
	// reviewVerdictPublicationIdentity's formula (as already happened once,
	// for issue #140) is only safe to apply to a reviewer that has never yet
...
func findExistingReviewVerdictComment(
	...
	for _, comment := range comments {
		...
		// reviewer's publication (issue #140: the ID is derived only from
		// repo/PR/head SHA, so two independent reviewers reaching
