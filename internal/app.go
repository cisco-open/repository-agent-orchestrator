...
// currently names it. ActiveReviewAgentID and a
...
func (b *Orchestrator) CleanupAgent(ctx context.Context, agentID string) error {
	...
	if agent.Role == RoleReviewer {
		...
		// this reviewer's own agent record is never purged. This exact gap
		// needed a manual state-file
		...
		// narrower, transient-failure-triggered path. See review feedback.
...
func (b *Orchestrator) pollActiveAgent(ctx context.Context, agentID string, now time.Time) {
	...
	if agent.Role == RoleReviewer &&
		...
		// active work.
		//
		// Gated on StateWorking specifically: ReviewCycle is
