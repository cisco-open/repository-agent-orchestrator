...
var (
	...
	craigBotAgentIDPattern   = regexp.MustCompile(`(?mi)^CODEX_AGENT_ID:\s*([^\s]+)\s*$`)
	craigBotAgentRolePattern = regexp.MustCompile(`(?mi)^CODEX_AGENT_ROLE:\s*([^\s]+)\s*$`)
)
...
func parseOrchestratorAgentID(text string) (string, bool) {
	match := craigBotAgentIDPattern.FindStringSubmatch(strings.TrimSpace(text))
	if len(match) != 2 {
		return "", false
	}
...
func parseOrchestratorAgentRole(text string) (AgentRole, bool) {
	match := craigBotAgentRolePattern.FindStringSubmatch(strings.TrimSpace(text))
	if len(match) != 2 {
		return "", false
	}
...
func (b *Orchestrator) ensureReviewAgentForCoder(ctx context.Context, coder Agent) error {
	...
	if activeReviewerID != "" {
		...
		if ok && reviewer.Role == RoleReviewer && !reviewer.Stopped && reviewer.State != StateDone && reviewer.State != StateErrored && reviewer.State != StateStopped {
			...
			// reviewer's own State to terminal (issue #148). Retiring and
...
// entire gate duration. See issue #138.
...
func (b *Orchestrator) prepareReviewGateLaunch(ctx context.Context, target reviewLaunchTarget, reviewer *Agent) error {
	// See issue #139 (review feedback on #138's fix): splitting the
...
func (b *Orchestrator) runReviewGateAndCoordinatorStartup(ctx context.Context, target reviewLaunchTarget, reviewer *Agent) error {
	// See the comment on prepareReviewGateLaunch's own defer (issue #139):
